...
function Network(input, output, createTopology = true) {
...
Network.prototype = {
    ...
    mutate: function (method) {
        ...
        const mutationFinish = performance.now();
...
Network.crossOver = function (network1, network2, equal) {
    ...
    switch (crossoverMethod) {
         ...
         * and designated a 'crossover point'. Bits to the right of
         * that point are swapped between the two parent chromosomes.
         * This results in two offspring, each carrying some genetic
         * information from both parents.
         */
        case methods.crossover.SINGLE_POINT: {


            const getHiddenNodeCount = (network) => {
                return network.nodes.length - network.input - network.output;
            };
            ...
            network1.nodes.map((node, index) => node.index = index);
            network2.nodes.map((node, index) => node.index = index);

            const nOHNodeCount = getHiddenNodeCount(network1) + network1.output;
            const nTHNodeCount = getHiddenNodeCount(network2) + network2.output;

            const minNodes = network1.input;
            const maxNodes = Math.min(nOHNodeCount, nTHNodeCount);
            ...
            // Initialise the new networks
            let childOne = new Network(network1.input, network1.output, false);


            let childTwo = new Network(network1.input, network1.output, false);
            childTwo.nodes = [];
            childTwo.connections = [];

            // Put back the input nodes
            childOne.nodes = network1.nodes.slice(0, crossoverPoint);
            childTwo.nodes = network2.nodes.slice(0, crossoverPoint);

            // Add the hidden nodes
            childOne.nodes = childOne.nodes.concat(networkTwoNodes);
            childTwo.nodes = childTwo.nodes.concat(networkOneNodes);

            // Put back the output nodes
            const networkOneOutputNodes = network1.nodes.slice(network1.nodes.length - network1.output, network1.nodes.length);
            const networkTwoOutputNodes = network2.nodes.slice(network2.nodes.length - network2.output, network2.nodes.length);

            childOne.nodes = [...childOne.nodes, ...networkOneOutputNodes];
            childTwo.nodes = [...childTwo.nodes, ...networkTwoOutputNodes];


            // Add the connections

            if (childOne.nodes.length !== (childOne.input + childOne.output)) {

                for (let i = 0; i <= childOne.input - 1; i++) {
                    for (let j = childOne.input; j <= (childOne.nodes.length - 1) - childOne.output; j++) {

                        let inputNode = childOne.nodes[i];
                        let hiddenNode = childOne.nodes[j];

                        const weight = Math.random() * this.input * Math.sqrt(2 / this.input);

                        childOne.connect(inputNode, hiddenNode, weight);
                    }
                }

            } else {

                for (let i = 0; i < childOne.input; i++) {
                    for (let j = childOne.input; j < childOne.output + childOne.input; j++) {

                        let weight = Math.random() * childOne.input * Math.sqrt(2 / childOne.input);
                        childOne.connect(childOne.nodes[i], childOne.nodes[j], weight);
                    }
                }

            }

            if (childTwo.nodes.length !== (childTwo.input + childTwo.output)) {

                for (let i = 0; i <= childTwo.input - 1; i++) {
                    for (let j = childTwo.input; j <= (childTwo.nodes.length - 1) - childTwo.output; j++) {

                        let inputNode = childTwo.nodes[i];
                        let hiddenNode = childTwo.nodes[j];

                        const weight = Math.random() * childOne.input * Math.sqrt(2 / childOne.input);

                        childTwo.connect(inputNode, hiddenNode, weight);
                    }
                }

            } else {

                for (let i = 0; i < childTwo.input; i++) {
                    for (let j = childTwo.input; j < childTwo.output + childTwo.input; j++) {

                        let weight = Math.random() * childTwo.input * Math.sqrt(2 / childTwo.input);
                        childTwo.connect(childTwo.nodes[i], childTwo.nodes[j], weight);
                    }
                }

            }

            offspring = (Math.floor(Math.random() * 11 <= 0.5)) ? childOne : childTwo;
            //console.log(offspring);
            break;
        }

        /**
         * In two-point crossover, two crossover points are picked
         * randomly from the parent chromosomes. The bits in between
         * the two points are swapped between the parent organisms.
         */
        case methods.crossover.TWO_POINT: {

            break;
        }

        /**
         * In uniform crossover, each bit from the offspring's genome
         * is independently chosen from the two parents according to
         * a given distribution.
         */
        case methods.crossover.UNIFORM: {
            //console.log("UNIFORM");

            // Initialise offspring
            offspring = new Network(network1.input, network1.output);
