class Player {
    ...
    updateMod(targetX, targetY, targetVX ,targetVY) {

        let input = this.detectMod(targetX, targetY, targetVX ,targetVY);
        let output = this.brain.activate(input);
        ...
        this.scoreMod(targetX, targetY, targetVX ,targetVY);
    ...
    scoreMod(targetX, targetY, targetVX ,targetVY) {

        const dist = window['p'].distance(this.x, this.y, targetX, targetY);
    ...
    detectMod(targetX, targetY, targetVX, targetVY) {

        let dist = Math.sqrt(this.x, this.y,targetX, targetY) / Math.sqrt(this.WIDTH ** 2 + this.HEIGHT ** 2);
        let targetAngle = window['p'].angleToPoint(this.x, this.y, targetX, targetY) / window['p'].TWO_PI;

        let vx = (this.vx + this.MAX_SPEED) / this.MAX_SPEED;
        let vy = (this.vy + this.MAX_SPEED) / this.MAX_SPEED;

        let tvx = (targetVX + this.MAX_SPEED) / this.MAX_SPEED;
        let tvy = (targetVY + this.MAX_SPEED) / this.MAX_SPEED;
